...
class FaqComponent extends StatelessComponent {
  ...
  Iterable<Component> build(BuildContext context) sync* {
    yield DomComponent(tag: 'html', children: [
      ...
      DomComponent(tag: 'body', attributes: {'class': 'bg-[#0A0F24] text-white'}, children: [
        DomComponent(tag: 'div', attributes: {'class': 'max-w-4xl mx-auto py-12 px-4'}, children: [
          DomComponent(tag: 'h1', attributes: {'class': 'text-4xl font-bold text-center mb-8'}, children: [
            text('FAQs')
          ]),
          DomComponent(tag: 'div', attributes: {'class': 'space-y-8'}, children: [
            DomComponent(tag: 'div', attributes: {'class': 'faq-item'}, children: [
              DomComponent(tag: 'div', attributes: {'class': 'flex justify-between items-center cursor-pointer'}, children: [
                DomComponent(tag: 'h2', attributes: {'class': 'text-xl font-bold'}, children: [
                  text('If the project is going to be based on ERC1155, why are you releasing your token on Binance Smart Chain?')
                ]),
                DomComponent(tag: 'i', attributes: {'class': 'fas fa-plus text-xl'})
              ]),
              DomComponent(tag: 'p', attributes: {'class': 'mt-4 text-base hidden'}, children: [
                text('Our mission with this project is to make real estate investments accessible to everyone. Right now, the fees on the Ethereum network are unaffordable for a large number of people. They are working to resolve the problem but till then we would rather put in extra work in the development phase by building a cross-chain swap, instead of excluding the people who cannot afford the current fees on the Ethereum network.')
              ])
            ]),
            DomComponent(tag: 'hr', attributes: {'class': 'border-gray-600'}),
            DomComponent(tag: 'div', attributes: {'class': 'faq-item'}, children: [
              DomComponent(tag: 'div', attributes: {'class': 'flex justify-between items-center cursor-pointer'}, children: [
                DomComponent(tag: 'h2', attributes: {'class': 'text-xl font-bold'}, children: [
              ...
              DomComponent(tag: 'p', attributes: {'class': 'mt-4 text-base hidden'})
            ]),
            DomComponent(tag: 'hr', attributes: {'class': 'border-gray-600'}),
            DomComponent(tag: 'div', attributes: {'class': 'faq-item'}, children: [
              DomComponent(tag: 'div', attributes: {'class': 'flex justify-between items-center cursor-pointer'}, children: [
